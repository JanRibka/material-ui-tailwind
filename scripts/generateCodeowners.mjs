...
function getAreaMaintainers(area, packageName) {
  return Array.from(
    new Set([
      ...
      // JanRibka UI package owner is not added to individual components' owners
      // to reduce the number of PRs they'll get to review.
      ...(packageName === 'material-ui-tailwind' ? [] : packageOwners[packageName]),
    ]),
  )
    .map((name) => `@${name}`)
    .join(' ');
...
function processComponents(packageName) {
  const componentsDirectory = path.join(thisDirectory, `../packages/janribkaui-${packageName}/src`);
  ...
  for (const componentDirectory of componentDirectories) {
    ...
    if (componentArea) {
      ...
      const codeowners = `/packages/janribkaui-${packageName}/src/${componentDirectory}/ ${maintainers}`;
...
function processPackages() {
  return Object.entries(packageMaintainers)
    .map(
      ([packageName, maintainers]) =>
        `/packages/janribkaui-${packageName}/ @${maintainers.join(' @')}`,
    )
    .join('\n');
}
...
function run() {
  ...
  write('\n# Components - JanRibka UI\n');
  write(processComponents('material-ui-tailwind'));
